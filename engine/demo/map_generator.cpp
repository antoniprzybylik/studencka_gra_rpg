...
void place_tree(Map &map, int width, int x)
{
	...
	map[y][x] = BLOCK_LEAFS;
	map[y][x+1] = BLOCK_LEAFS;
	map[y][x-1] = BLOCK_LEAFS;
	map[(y-1)][x+1] = BLOCK_LEAFS;
	map[(y-1)][x-1] = BLOCK_LEAFS;
	
	for (i = 1; i <= h-3; i++) {
		map[(y-i-1)][x+1] = BLOCK_LEAFS;
		map[(y-i-1)][x-1] = BLOCK_LEAFS;
	}
}
...
void place_cloud(Map &map, int width, int x)
{
	...
	if (pick(re) & 1) {
		map[(y+4)][x] = BLOCK_RDCLD;
		map[(y+5)][x] = BLOCK_RUCLD;
		map[(y+4)][x+1] = BLOCK_LDCLD;
		map[(y+5)][x+1] = BLOCK_LUCLD;
	} else {
		map[(y+5)][x] = BLOCK_RDCLD;
		map[(y+6)][x] = BLOCK_RUCLD;
		map[(y+5)][x+1] = BLOCK_LDCLD;
		map[(y+6)][x+1] = BLOCK_LUCLD;
	}
}
